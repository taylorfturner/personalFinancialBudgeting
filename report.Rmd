...
for (uniqueTRID in unique(tmp$TRID)){
  ...
  plot <- ggplot() + geom_point(data = tridTmp, show.legend = c(color = FALSE), aes(x = PERIOD, y = sum, color = TRID_CODE, shape = as.factor(negPosFlg))) + facet_wrap(~TRID) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("Cumulative Time Series Sum of Balance by TRID")
  print (plot)
}
