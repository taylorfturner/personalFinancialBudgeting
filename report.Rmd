...
for (tbl in tbls){
  if(tbl == "R_MARGIN_VIEW"){
    ...
    sumar$flg <- as.factor(ifelse(sumar$MARGIN < 0, 'X<$0', ifelse(sumar$MARGIN > 0 & sumar$MARGIN < 1000, '$0<X<$1000', ifelse(sumar$MARGIN > 1000 & sumar$MARGIN < 1500,'$1000<X$1500', '$1500<X'))))
    sql <- NULL
  }
  ...
  if(tbl == "R_MARGIN_DECAY"){
    ...
    decay$month <- substring(decay$PERIOD,5,7)
  ...
  if(tbl == "R_HIST"){
    ...
    hist$debitnum <- as.numeric(as.character(hist$debit))
    hist$dynm <- as.numeric(hist$dynm)
    hist$MONTH <- as.numeric(hist$MONTH)
  ...
  if(tbl == "R_TRANS"){
    ...
    trans$period <- as.factor(trans$PERIOD)
    sql <- NULL
  }
  ...
  if(tbl == "R_SAVINGS"){
    ...
    savingproforma <- savingproforma %>% group_by(TRID_CODE) %>% mutate(runbal = cumsum(CREDIT))
...
catmarg$ROW_NUM <- NULL
catmarg$PERIOD <- NULL
catmarg$DEBIT <- NULL
catmarg$CREDIT <- NULL
stdevcatmarg <-catmarg %>% group_by(TRID) %>% mutate(stdev = sd(MARGIN))
stdevcatmarg$MARGIN <- NULL
...
#Savings Analysis
```{r saving rate}
tmp <- read.csv("budget_subsum_view.csv", header = TRUE)
tmp_one <- subset(tmp, TRID == 'SVTRID')
tmp <- tmp %>% group_by(PERIOD) %>% summarise(INCOME = sum(CREDIT))
tmp <- merge(tmp_one, tmp)
rm(tmp_one)
tmp_two <- subset(hist, hist$TRID == 'SVTRID')
tmp_two <- tmp_two %>% group_by(PERIOD, TRID) %>% summarise(savingextract = sum(as.numeric(as.character(debit)) * -1))
tmp <- merge(tmp, tmp_two, by = "PERIOD", all.x = TRUE)
tmp$savingextract <- replace(tmp$savingextract, is.na(tmp$savingextract), 0)
tmp$TRID.y <- NULL
rm(tmp_two)
tmp$CREDIT <- tmp$CREDIT - tmp$savingextract
tmp$savingextract <- NULL
tmp$rownum <- as.numeric(rownames(tmp))
tmp$savingrate <- tmp$CREDIT / tmp$INCOME
ggplot(tmp) + geom_bar(aes(x = rownum, y = savingrate, color = PERIOD), stat = "identity") + xlab("F(t)") + ylab("Savings Rate") + ggtitle("Actual Savings Rate by Period") + scale_y_continuous(breaks = seq(-.50,.50, by = .05))
tmp
rm(tmp)
```
...
for (prdRec in unique(decay$PERIOD)){
  ...
  if (!exists("slopeCoef")){
    ...
    slopeCoef <- setnames(data.frame(prdRec, as.numeric(coef(lm(sub_df$opmarg ~ sub_df$DAY))["sub_df$DAY"])), c("prd", "slope"))
    rm(sub_df)
  }
  ...
  if (exists("slopeCoef")){
    ...
    if(length(sub_df$PERIOD) >= 1){
      temp_datset <- setnames(data.frame(prdRec, as.numeric(coef(lm(sub_df$opmarg ~ sub_df$DAY))["sub_df$DAY"])), c("prd", "slope"))
...
slopeCoef <- slopeCoef[!is.na(slopeCoef$slope), ]

plot <- ggplot(slopeCoef) + geom_point(aes(x = prd, y = slope, color = prd)) + xlab("Period") + ylab("Slope Coefficient") + ggtitle("Slope Coefficient on lm(opmarg ~ DAY)") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplotly(plot)
```
