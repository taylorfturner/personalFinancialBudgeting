...
tmp <- summary_view %>% group_by(PERIOD, TRID) %>% summarise(sum = sum(cumsumVar))
tmp$negPosFlg <- ifelse(tmp$sum < 0, 0, 1)
ggplot() + geom_point(data = tmp, aes(x = PERIOD, y = sum, color = as.factor(negPosFlg))) + facet_wrap(~TRID) + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ggtitle("Cumulative Time Series Sum of Balance by TRID")
rm(tmp)
```
